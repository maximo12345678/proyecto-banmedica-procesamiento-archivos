...
# Se descarga a TMP la libreria para captura de texto desde el S3. 
...
def pageScanOrEmptyProcess(page, numPage):
    ...
    try:  
        ...
        # ]"
        prediction = pipeline.recognize(route)
...
# Importar la libreria desde el bucket   
def importModule():
    #s3 client
    s3 = boto3.resource('s3')
    bucket = os.environ['BUCKET2'] 
    # s3.Object(bucket, 'python.zip').download_file('/tmp/keras-ocr.zip')

    # replace with your bucket name
    KEY_ZIP = 'keras-ocr/python.zip' # replace with your object key
    TEMP_FILE_ZIP ='/tmp/keras-ocr.zip' # temp path to save file

    #to download file from S3 bucket
   
    try:
        s3.Bucket(bucket).download_file(KEY_ZIP, TEMP_FILE_ZIP)
        zfile = zipfile.ZipFile(TEMP_FILE_ZIP)
        
        # checking if file is downloaded
        print("Libreria descargada: ", os.path.isfile(TEMP_FILE_ZIP))
        
        with zipfile.ZipFile(TEMP_FILE_ZIP, 'r') as zip_ref:
            zip_ref.extractall('/tmp/extra_package')
            zip_ref.close()
        # list the extra_packages
        
        from subprocess import call
        call(['ls', '-l', '/tmp/extra_package'])
        
        #checking file is unzipped. All our dependencies are stored in /tmp.
        x = os.listdir("/tmp/extra_package/python")
        
        #since our dependencies were stored in python folder
        print("Dependencias: ", x)
        
        sys.path.insert(0, '/tmp/extra_package/python') #adding this path for our python to import from

    except (Exception, psycopg2.Error) as error:
        print("Error en descarga de libreria: ", error)
